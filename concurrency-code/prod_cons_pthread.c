 ...
 *
Questo codice implementa un semplice esempio di produttore-consumatore
utilizzando thread in C. Il produttore genera elementi e li inserisce in un
buffer condiviso, mentre il consumatore preleva gli elementi dal buffer. Il
buffer ha una dimensione massima definita da BUFFER_SIZE. Vengono utilizzati
mutex e variabili condizionali per sincronizzare l’accesso al buffer e garantire
che il produttore attenda quando il buffer è pieno e il consumatore attenda
...
Il mutex viene creato con attributi predefiniti (non-ricorsivo, non-condiviso
tra processi)
